...
const CarouselTemplate = ({ Data }: { Data: CarouselTemplateData["data"] }) => {
  ...
  const imageLink =
    isDarkMode && noLogos[currentIndex].image?.imageDarkMode
      ...
      : noLogos[currentIndex].image?.imageLink;

  const oneImage = noLogos.length === 1;
    ...
    <div className="w-full flex flex-col justify-center items-center px-4 mb-10">
      <div className="relative group w-full max-w-4xl">
        <div className="flex justify-between gap-5 items-center">
          ...
          {noLogos[currentIndex].image && (
            <ImageTemplate
              image={imageLink || ""}
              name={noLogos[currentIndex].image.imageName || ""}
              type={"Banner"}
            />
          )}
          {!oneImage && (
      ...
      <div className="dark:text-dark-dsmlcBlack text-xl text-light-dsmlcBlack pt-5 flex flex-col gap-5 text-center">
        {noLogos[currentIndex].header}
        {noLogos[currentIndex].points?.map((point, index) => (
